...
| `sdk_login.cjs` | Node.js | Trigger a native SDK login to generate capturable traffic |
| `test_sha1.mjs` | Node.js | Call `PUB_SHA1Encrypt` from the SDK to compare with Python output |
...
2. Copy capture_sdk.sh + sdk_login.cjs into the container
...
bridges/            ← supported runtime (Node SDK bridge)
...
Node.js + koffi) and are intended for manual, interactive use only.
